defmodule ExGrib.Grib2.Section4.Templates.PostProcessingAverageAccumulationExtremeValuesAtIntervals do
  @moduledoc """
  Template 4.72

  Post processing average accumulation extreme values at intervalsstatistically processed values at a horizontal level or in a horizontal layer in a continuous or non-continuous time interval
  ...
  measured by analogue maximum and minimum thermometers or thermographs, and the rainfall measured by a rain gauge.
  (7) The reference and forecast times are successively set to their initial values plus or minus the increment, as
  defined by the type of time increment (one of octets 63, 65, 77, ...). For all but the innermost (last) time range,
  the next inner range is then processed using these reference and forecast times as the initial reference and forecast times.

  WARNING THIS TEMPLATE WAS AUTOGENERATED AND IS LIKELY INCOMPLETE

  https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_temp4-72.shtml
  """
  ...
  def get(<<
        ...
        # Time increment between successive fields in units defined by the previous octet (see Notes 6 and 7)
